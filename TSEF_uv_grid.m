...
for ii=1:Nt
    ...
    for ki=1:n_o
        ...
        % Step 1: Get p, u, v increments
            ...
            theta = mod(atan2(v,u) - (pi/16)*theta_obs(ki,kj,ii),2*pi);
            ...
            theta = theta + (pi/16)*theta_obs(ki,kj,ii);
        ...
        % Step 2: Conditional sampling via regression
            ...
            % Use u, v to get regression coefficients 
            Z = [ones(Ne,1) u(:) v(:)];
            beta = shiftdim(reshape(Z\reshape(shiftdim(Q,2),Ne,[]),[3 256 256]),1);
            % Find linear regression increments
            Q_inc = bsxfun(@times,beta(:,:,2),u_inc)...
                  + bsxfun(@times,beta(:,:,3),v_inc);
