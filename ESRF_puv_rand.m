...
for ii=1:Nt
    ...
    % Assimilate p
        ...
        % Update mean
        Q = Q + reshape(bsxfun(@times,loc1(:),(A*(V'))*(psi_obs(ko,ii)-mean(p))*(1/(s2 + g2))),[256 256]);
    ...
    % Assimilate normal component of wind
        for n=1:Ne
            ...
            nw(n) = cos((pi/16)*theta_obs(ko,ii))*v(n) - sin((pi/16)*theta_obs(ko,ii))*u(n);
        end
        % Ensemble perturbation matrix
        A = reshape(Q-mean(Q,3),[],Ne)/sqrt(Ne-1);
        % Ensemble obs perturbation matrix, cos(theta)
        V = (nw - mean(nw))'/sqrt(Ne-1);
        s2 = V*(V'); g2 = 4*mean(u.^2+v.^2)*0.009683; % prior and likelihood variances in obs space
        % Update mean
