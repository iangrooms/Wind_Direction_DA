...
u = zeros(Ne,1);
v = zeros(Ne,1);
tw = zeros(Ne,1); % tangetial component of wind
nw = zeros(Ne,1); % normal component of wind
p = zeros(Ne,1);
...
for ii=1:Nt
    ...
    for ki=1:n_o
        ...
        % Assimilate normal component of wind
            for n=1:Ne
                psi_hat(:,:,n) = get_psi(fft2(Q(:,:,n)),params);
                tmp = real(ifft2(-1i*KY.*psi_hat(:,:,n)));
                u(n) = tmp(ind_o(ki),ind_o(kj));
                tmp = real(ifft2( 1i*KX.*psi_hat(:,:,n)));
                v(n) = tmp(ind_o(ki),ind_o(kj));
                nw(n) = cos((pi/16)*theta_obs(ki,kj,ii))*v(n) - sin((pi/16)*theta_obs(ki,kj,ii))*u(n);
            end
            % Ensemble perturbation matrix
            A = reshape(Q-mean(Q,3),[],Ne)/sqrt(Ne-1);
            % Ensemble obs perturbation matrix, cos(theta)
            V = (nw - mean(nw))'/sqrt(Ne-1);
            s2 = V*(V'); g2 = mean(u.^2+v.^2)*0.009683; % prior and likelihood variances in obs space
            % Update mean
            Q = Q + reshape(bsxfun(@times,loc1(:),(A*(V'))*(0 - mean(nw))*(1/(s2 + g2))),[256 256]);
            % Update perturbations
            WHB = 1/(s2 + g2 + sqrt(s2*g2 + g2^2)); % Whitaker & Hamil Beta
            Q = Q - WHB*sqrt(Ne-1)*reshape(bsxfun(@times,loc1(:),A*(V')*V),[256 256 Ne]);
        % Assimilate tangential component of wind
%             for n=1:Ne
%                 psi_hat(:,:,n) = get_psi(fft2(Q(:,:,n)),params);
%                 tmp = real(ifft2(-1i*KY.*psi_hat(:,:,n)));
%                 u(n) = tmp(ind_o(ki),ind_o(kj));
%                 tmp = real(ifft2( 1i*KX.*psi_hat(:,:,n)));
%                 v(n) = tmp(ind_o(ki),ind_o(kj));
%                 tw(n) = cos((pi/16)*theta_obs(ki,kj,ii))*u(n) + sin((pi/16)*theta_obs(ki,kj,ii))*v(n);
%             end
%             if(any(tw<0))
%                 % Ensemble perturbation matrix
%                 A = reshape(Q-mean(Q,3),[],Ne)/sqrt(Ne-1);
%                 % Ensemble obs perturbation matrix, cos(theta)
%                 V = (tw - mean(tw))'/sqrt(Ne-1);
%                 s2 = V*(V'); g2 = mean(u.^2+v.^2); % prior and likelihood variances in obs space
%                 % Update mean
%                 Q = Q + reshape(bsxfun(@times,loc1(:),(A*(V'))*(sqrt(mean(u.^2+v.^2)) - mean(tw))*(1/(s2 + g2))),[256 256]);
%                 % Update perturbations
%                 WHB = 1/(s2 + g2 + sqrt(s2*g2 + g2^2)); % Whitaker & Hamil Beta
%                 Q = Q - WHB*sqrt(Ne-1)*reshape(bsxfun(@times,loc1(:),A*(V')*V),[256 256 Ne]);
%             end
...
%     clf
%     subplot(1,2,1)
%     imagesc(AM(:,:,ii)-FM(:,:,ii)),axis image,colorbar
%     hold on
%     spy(obs_mask,'.k')
%     subplot(1,2,2)
%     imagesc(AS(:,:,ii)./FS(:,:,ii)),axis image,colorbar
%     hold on
%     spy(obs_mask,'.k')
%     title(num2str(ii))
%     pause(0.1)
